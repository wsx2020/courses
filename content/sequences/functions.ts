...
function geometric(a: number, q: number, n: number) {
  return tabulate((i) => a * Math.pow(q, i), n);
}
...
export function arithmeticGeometricGraph($step: Step) {
  ...
  $step.model.geometric =
      (a: number, q: number, i: number) => numberFormat(a * (q ** i), 4);

  $step.model.watch((m: any) => {
    const p1 = arithmetic(m.a, m.d, 10).map((p, i) => new Point(1 + i, p));
    const p2 = geometric(m.b, m.q, 10).map((p, i) => new Point(1 + i, p));
  ...
  $actions[0].on('click', () => $step.model.assign({b: 2, q: 2}));
  $actions[1].on('click', () => $step.model.assign({b: 10, q: 0.6}));
  $actions[2].on('click', () => $step.model.assign({b: 3, q: -1.4}));
}
